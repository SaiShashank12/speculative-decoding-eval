...
\paragraph{Med-Gemma Initial Attempt.}
Our original proposal planned to evaluate speculative decoding on Med-Gemma \citep{medgemma2024} for medical NLP tasks. Initial attempts to load Med-Gemma failed due to gated repository access restrictions and dependency version conflicts. After resolving authentication and upgrading the HuggingFace pipeline, we successfully loaded and benchmarked \texttt{google/medgemma-4b-it} (4B parameters) on Apple Silicon MPS, achieving 20.47 tokens/sec. While Med-Gemma is now functional for vanilla inference, applying speculative decoding to it remains a challenge: no pre-trained EAGLE weights exist for Gemma-3-based architectures, and finding an appropriate smaller draft model within the same architecture family is an open problem we plan to address.
...
\begin{enumerate}
    ...
    \item \textbf{Med-Gemma speculative decoding:} Investigate draft model candidates for Med-Gemma 4B (e.g., Gemma-3-1B or distilled variants) to enable speculative decoding benchmarks on a domain-specific medical model.
\end{enumerate}
